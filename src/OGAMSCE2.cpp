	...
	*(textBuffer.reserve(1)) = '\0';
	...
	while(1)
	{
				...
				for( slot = 0; slot < scrollBar.view_size; ++slot)
				{
					...
					if( refreshFlag & TUOPTION_BROWSE(slot) )
					{
						...
						if( rec >= 1 && rec <= scenCount )
						{
                            ...
                            if (scenInfoArray[rec - 1].status == ScenInfo::COMPLETED)
                            {
                            ...
                            else                                                    // Empty checkbox
                            {
                                ...
                                if (scenInfoArray[rec - 1].status == ScenInfo::PLAYED)   // Line in the middle
                                {
		...
		else if( startButton.detect() )
		{
            ...
            // mark as played
            if (scenInfoArray[retFlag - 1].status == ScenInfo::Status::UNPLAYED) {
                scenInfoArray[retFlag - 1].status = ScenInfo::Status::PLAYED;
